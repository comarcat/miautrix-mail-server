...
Write-Host "[2/7] Publishing .NET 10 backend (Self-Contained linux-x64)..." -ForegroundColor Yellow
dotnet publish src/Miautrix.Mail.Web/Miautrix.Mail.Web.csproj -c Release -r linux-x64 --self-contained true -o $AppDir

# 3. Publish outbound worker
Write-Host "[3/7] Publishing outbound worker (Self-Contained linux-x64)..." -ForegroundColor Yellow
...
Write-Host "[4/7] Building Admin Frontend (React)..." -ForegroundColor Yellow
...
# 5. Build Webmail Frontend
Write-Host "[5/7] Building Webmail Frontend (React)..." -ForegroundColor Yellow
...
# 6. Copy to Target Container via SCP
Write-Host "[6/7] Deploying to LXC container ($TargetIp)..." -ForegroundColor Green

Write-Host " -> Preparing target directories and stopping running service..."
ssh "root@$($TargetIp)" "systemctl stop miautrix-mail miautrix-mail-worker || true; mkdir -p /opt/miautrix-mail/app /opt/miautrix-mail/worker /opt/miautrix-mail/admin /opt/miautrix-mail/webmail"

Write-Host " -> Copying Backend Application..."
scp -r (Join-Path $AppDir "*") "root@$($TargetIp):/opt/miautrix-mail/app/"

Write-Host " -> Copying Worker Application..."
scp -r (Join-Path $WorkerDir "*") "root@$($TargetIp):/opt/miautrix-mail/worker/"
...
ssh "root@$($TargetIp)" "chmod +x /opt/miautrix-mail/app/Miautrix.Mail.Web /opt/miautrix-mail/worker/Miautrix.Mail.Worker && chown -R www-data:www-data /opt/miautrix-mail && systemctl restart miautrix-mail miautrix-mail-worker nginx"

# 7. Service Verification
Write-Host "[7/7] Verifying Service Endpoints..." -ForegroundColor Yellow
